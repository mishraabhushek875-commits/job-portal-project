...
import Application from '../models/application.js'; // To count applicants
...
export const createJob = async (req, res) => {
  try {
    ...
    const job = await Job.create({
      ...
      // ✅ Yeh sahi hai — array bhi handle karo, string bhi
skills: Array.isArray(req.body.skills)
  ? req.body.skills                        // already array hai — seedha use karo
  : req.body.skills?.split(',').map(s => s.trim()) || []  // string hai — split karo
  
  //yha par sirf string m ehi sikkls aayegi to usko array me convert krne ke liye split kr rhe hai
...
// Yeh purana block dhundo aur replace karo
if (req.user && keyword) {
  SearchHistory.create({
    ...
    console.log('✅ Search history saved | user:', req.user.id, '| keyword:', keyword);
  }).catch(err => {
...
// jab hum normal req.body krte hai tab  job.recruiter = "64f8a2b1c3d4e5f6a7b8c9d0" Sirf ID hai — naam kahan se laayein?
    ...
    const applicantsCount = await Application.countDocuments({ job: req.params.id });
